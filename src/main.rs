// region IO
//! Easy I/O utilities.
// https://jh05013.github.io/ps_snippets/io/struct.OJ.html
...
fn main() {
    let mut oj = stdin();
    for _tc in 1..=oj.i64() {
        let (n, m) = (oj.i64(), oj.i64());
        let mut v: VecDeque<(i64, bool)> = VecDeque::new();
        for i in 0..n {
            let j = oj.i64();
            let b : bool = i == m;
            v.push_back((j, b));
        ...
        loop {
            let mut mx = -1;
            for i in &v {
                mx = std::cmp::max(mx, i.0);
            }
            if v.front().expect("").0 == mx {
                cur += 1;
                if v.front().expect("").1 {
                    oj.write(cur).ln();
                    break;
